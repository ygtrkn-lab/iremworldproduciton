...
export default function StoreDetailPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-50/50">
            ...
            <div className="bg-gradient-to-br from-[#f07f38] to-[#d65a1b] rounded-xl p-4 sm:p-5 text-white">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                ...
                {store.stats.employeeCount && (
                  <div>
                    ...
                    <span className="flex items-center gap-1">
                      ...
                      {store.contact.city}, {store.contact.country}
                    </span>
                    {store.stats.employeeCount && (
                      <>
                        <span>•</span>
                        <span>{store.stats.employeeCount} çalışan</span>
                      </>
                    )}
                  </div>
                  <div className="flex flex-wrap gap-2 mt-3">
                    {store.verified && (
                      <span className="inline-flex items-center gap-1 bg-primary-50 text-primary-700 text-xs font-semibold px-3 py-1 rounded-full">
                ...
                <div className="flex gap-3">
                  <a
                    href={`mailto:${store.contact.email}`}
                    className="px-6 py-2.5 bg-primary-600 hover:bg-primary-700 text-white font-semibold rounded-full transition-colors flex items-center gap-2"
                  >
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                    </svg>
                  ...
                  {store.contact.website && (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="px-6 py-2.5 border-2 border-primary-600 text-primary-600 hover:bg-primary-50 font-semibold rounded-full transition-colors"
                    >
                      Website
                    </a>
                  )}
          ...
          <div className="border-t border-gray-200 -mx-4 px-4">
            <div className="flex items-center gap-8 py-4 overflow-x-auto">
              <div className="flex items-center gap-2 text-sm">
                <span className="font-semibold text-gray-900">{store.stats.activeListings}</span>
                <span className="text-gray-600">Aktif İlan</span>
              </div>
              <div className="w-px h-4 bg-gray-300" />
              <div className="flex items-center gap-2 text-sm">
                <span className="font-semibold text-gray-900">{store.stats.yearsInBusiness}</span>
              ...
              {store.stats.totalSales && (
                <>
                  <div className="w-px h-4 bg-gray-300" />
                  <div className="flex items-center gap-2 text-sm">
                    <span className="font-semibold text-gray-900">{store.stats.totalSales}</span>
                    <span className="text-gray-600">Başarılı Satış</span>
                  </div>
                </>
              )}
              {store.rating && (
                <>
                  ...
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    {store.reviewCount && (
                      <span className="text-gray-600">({store.reviewCount} değerlendirme)</span>
                    )}
                  </div>
                </>
              )}
      ...
      {/* Content */}
      <div className="container mt-6 pb-20">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* Sidebar */}
          <div className="lg:col-span-1 space-y-4">
            {/* About Card - LinkedIn Style */}
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              <h3 className="text-lg font-bold text-gray-900 mb-4">Hakkımızda</h3>
              ...
              {store.legalName && (
                <div className="pt-4 border-t border-gray-100">
                  <p className="text-xs text-gray-500 mb-1">Ticari Ünvan</p>
                  <p className="text-sm font-medium text-gray-900">{store.legalName}</p>
                </div>
              )}
              {store.foundedYear && (
                <div className="pt-4 border-t border-gray-100 mt-4">
                  <p className="text-xs text-gray-500 mb-1">Kuruluş Yılı</p>
                  <p className="text-sm font-medium text-gray-900">{store.foundedYear}</p>
                </div>
              )}
            </div>
            ...
            {/* Specialties Card */}
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              <h3 className="text-lg font-bold text-gray-900 mb-4">Uzmanlık Alanları</h3>
              <div className="flex flex-wrap gap-2">
                {store.specialties.map(spec => (
                  <span
                    key={spec}
                    className="bg-primary-50 text-primary-700 px-3 py-1.5 rounded-full font-medium text-sm"
                  >
                    {spec}
                  </span>
                ))}
            ...
            {/* Service Areas Card */}
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              <h3 className="text-lg font-bold text-gray-900 mb-4">Hizmet Bölgeleri</h3>
                ...
                <div className="flex items-center gap-3">
                  <svg className="w-5 h-5 text-gray-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path
                      ...
                      strokeLinejoin="round"
                      strokeWidth={2}
                      d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21L6.16 10.928c-.652.35-.852 1.17-.43 1.768C6.98 14.528 9.472 17.02 11.304 18.27c.598.422 1.418.222 1.768-.43l1.541-4.064a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.948V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"
                    />
                  </svg>
                  <a
                    href={`tel:${store.contact.phone}`}
                ...
                {store.contact.website && (
                  <div className="flex items-center gap-3">
                    ...
                    <a
                      ...
                      className="text-sm text-primary-600 hover:underline"
                    >
                      {store.contact.website.replace(/^https?:\/\//, "")}
                    </a>
                  </div>
                )}
              </div>
                  ...
                  <div className="flex gap-2">
                    {store.social.instagram && (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="w-9 h-9 flex items-center justify-center rounded-lg bg-gradient-to-br from-purple-500 to-pink-500 text-white hover:shadow-lg transition-shadow"
                      >
                        <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                    ...
                    {store.social.facebook && (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="w-9 h-9 flex items-center justify-center rounded-lg bg-blue-600 text-white hover:shadow-lg transition-shadow"
                      >
                        <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                    ...
                    {store.social.linkedin && (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="w-9 h-9 flex items-center justify-center rounded-lg bg-blue-700 text-white hover:shadow-lg transition-shadow"
                      >
                        <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                          <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
                        </svg>
                    ...
                    {store.social.twitter && (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="w-9 h-9 flex items-center justify-center rounded-lg bg-gray-900 text-white hover:shadow-lg transition-shadow"
                      >
                        <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
          ...
          <div className="lg:col-span-2 space-y-6">
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                  ...
                  {properties.slice(0, 6).map(property => (
                    ...
                    >
                      <div className="relative h-44">
                        ...
                        />
                        <div className="absolute top-2 right-2 bg-white px-2.5 py-1 rounded-md text-sm font-bold text-primary-600 shadow-sm">
                      ...
                      <div className="p-4">
                        ...
                        <div className="flex items-center gap-2 text-xs text-gray-500">
                          <span>{property.specs.rooms}</span>
                          <span>•</span>
                          <span>{property.specs.netSize} m²</span>
                          <span>•</span>
                          <span>{property.category.sub}</span>
                        </div>
                      </div>
                    </Link>
                  ))}
                ...
                {properties.length > 6 && (
                  <div className="mt-4 pt-4 border-t border-gray-100 text-center">
                    <Link
                      href={`/property?store=${store.id}`}
                      className="text-primary-600 hover:text-primary-700 font-semibold text-sm"
                    >
                      Tüm İlanları Görüntüle ({properties.length}) →
                    </Link>
                  </div>
                )}
              </div>
            ...
            {projects.length > 0 && (
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <div className="flex items-center justify-between mb-6">
                  <h2 className="text-xl font-bold text-gray-900">Projelerimiz</h2>
                  <span className="bg-primary-50 text-primary-700 px-3 py-1 rounded-full text-sm font-semibold">
                    ...
                    >
                      <div className="relative h-44">
                        <img
                          ...
                          className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300"
                        />
                        <div className="absolute top-2 left-2 bg-primary-600 text-white px-3 py-1 rounded-md text-xs font-semibold shadow-sm">
                          {project.status}
                        </div>
                      </div>
                      <div className="p-4">
                ...
                {projects.length > 4 && (
                  <div className="mt-4 pt-4 border-t border-gray-100 text-center">
                    <Link
                      href={`/projects?store=${store.id}`}
                      className="text-primary-600 hover:text-primary-700 font-semibold text-sm"
                    >
                      Tüm Projeleri Görüntüle ({projects.length}) →
                    </Link>
                  </div>
                )}
              </div>
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <div className="flex items-center justify-between mb-6">
                  <div>
                    ...
                    <p className="text-sm text-gray-600 mt-1">Size yardımcı olabilecek danışmanlarımız</p>
                  </div>
                  <span className="bg-primary-50 text-primary-700 px-3 py-1 rounded-full text-sm font-semibold">
                    {store.employees.length} Kişi
                  </span>
                </div>
                <div className="space-y-4">
                  {store.employees.map(employee => (
                    <div
                      key={employee.id}
                      className="flex gap-4 p-4 border border-gray-200 rounded-xl hover:border-primary-300 hover:bg-primary-50/30 transition-all"
                    >
